...
export default function RequestDetailRoute() {
  const { familyId, requestId } = useLocalSearchParams<{ familyId: string; requestId: string }>();
  const queryClient = useQueryClient();
  const requests = useQuery({ queryKey: ["requests", familyId], queryFn: () => api.requests(familyId), enabled: Boolean(familyId) });
  const request = requests.data?.find((item) => item.id === requestId);
  const decide = useMutation({ mutationFn: (decision: "approve" | "deny") => api.decideRequest(familyId, requestId, decision, "Reviewed in request detail"), onSuccess: () => void queryClient.invalidateQueries({ queryKey: ["requests", familyId] }) });
