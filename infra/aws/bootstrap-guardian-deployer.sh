...
# a Guardian-only deployment role; all provisioning then assumes that role.
...
if ! aws iam get-role --role-name "$role_name" >/dev/null 2>&1; then
  aws iam create-role \
    --role-name "$role_name" \
    --description "Limited CloudFormation deployment role for Guardian production MVP" \
...
echo "Guardian deployment role ready: arn:aws:iam::${account_id}:role/${role_name}"
