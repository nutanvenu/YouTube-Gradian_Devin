...
| §2.1 G5 parent UX and three-interaction law | Parent routes, direct Home actions, dedicated detail routes and design system | `.scratch/emulator/interaction-audit/README.md`; `audit.txt`; `parent-home-add-time-pause.png`; `live-policy-action-evidence.txt`; mobile typecheck | Implemented locally | iOS/split-view and physical-device acceptance remain external. |
...
| §26 screen inventory | Dedicated parent/child routes and tablet layouts | `.scratch/emulator/interaction-audit/README.md`; `audit.txt`; route source under `apps/mobile/src/app`; tablet screenshots under `.scratch/emulator/tablet-*` | Implemented locally | iPad/split-view requires Mac/Xcode. |
| §27 three-interaction compliance matrix | Parent direct Home actions, dedicated routes and child request/time surfaces | `.scratch/emulator/interaction-audit/README.md`; `audit.txt`; `parent-home-add-time-pause.png`; `push-fixture-evidence.txt`; focused backend/mobile tests | Implemented locally | APNs/FCM transport receipt and physical-device acceptance remain external. |
